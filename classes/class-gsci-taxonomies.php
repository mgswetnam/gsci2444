...
/**
 * Custom taxonomy factory for Canadensis
 * @class GSCI_Canadensis_Taxonomies
 */

final class GSCI_Canadensis_Taxonomies {
  ...
  public function __construct() {
	...
	static public function gsci_add_custom_taxonomy() {
    include FL_CHILD_THEME_DIR ."/includes/taxonomies.php";
    if( !empty( $taxonomies ) ){
      foreach( $taxonomies as $taxonomy ){
        ...
        $description = ( ( array_key_exists( "description", $taxonomy ) )? __( $taxonomy[ "description" ], 'gsci_canadensis_'.strtolower( $singular ) ) : false );
  ...
  static public function define_taxonomy_labels( $singular, $plural ) {
    $labels = array(
      'name' => __( ucwords( $plural ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'singular_name' => __( ucwords( $singular ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'menu_name' => __( ucwords( $plural ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'all_items' => __( 'All '.ucwords( $plural ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'edit_item' => __( 'Edit '.ucwords( $singular ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'view_item' => __( 'View '.ucwords( $singular ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'update_item' => __( 'Update '.ucwords( $singular ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'add_new_item' => _x( 'Add New', strtolower( $singular ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'new_item_name' => _x( 'New', strtolower( $singular ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'parent_item' => __( 'Parent '.ucwords( $singular ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'parent_item_colon' => __( 'Parent '.ucwords( $plural ).':', 'gsci_canadensis_'.strtolower( $singular ) ),
      'search_items' => __( 'Search '.ucwords( $plural ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'popular_items' => __( 'Popular '.ucwords( $plural ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'separate_items_with_commas' => __( 'Separate '.ucwords( $plural ).' With Commas', 'gsci_canadensis_'.strtolower( $singular ) ),
      'add_or_remove_items' => __( 'Add or Remove '.ucwords( $plural ), 'gsci_canadensis_'.strtolower( $singular ) ),
      'choose_from_most_used' => __( 'Choose '.ucwords( $singular ).' From Most Used', 'gsci_canadensis_'.strtolower( $singular ) ),
      'not_found' => __( 'No '.strtolower( $plural ).' found.', 'gsci_canadensis_'.strtolower( $singular ) ),
		);
    return $labels;
  }

  static public function gsci_add_taxonomy_filter(){
    global $typenow;
    include FL_CHILD_THEME_DIR ."/includes/taxonomies.php";
    if( !empty( $taxonomies ) ){
      foreach( $taxonomies as $taxonomy ){
